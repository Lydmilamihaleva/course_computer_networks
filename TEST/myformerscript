	...
	switch(type) {
		case "router":
			if (routernumber>=1) {
				...
				console.log("i've just made this");
				//svg=document.getElementsByName('svg')
...
function Transfer(evt){ //...Передвижение;
	...
	function FinishingTransfer(evt)
	{
		...
		if (finx>500 || finy>500){
			...
			var dname=(d.getAttributeNS(null,"name"));
			d.setAttributeNS(null,"x",finx);
			d.setAttributeNS(null, "y", finy);
			//var dname=evt.target.getAttributeNS(null,"name");
			console.log(dname);
			ObjectsAvailable(dname);
			evt.target.setAttributeNS(null,"x",firstx);
			evt.target.setAttributeNS(null,"y",firsty);
			evt.target.setAttributeNS(null,"position",absolute);
